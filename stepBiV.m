function stepBiV(BiV,params,x,scaling,step,options)
% stepBiV   Updates Par for the current step in evalBiV
...
%% Sarcomere
currentSarc = ParRef.Lv.Sarc;

currentSarc.SfAct  = xStepCalc(x, ParRef.Lv.Sarc.SfAct,  step,options.NumSteps,scaling,params,'SfAct');
currentSarc.SfPas  = xStepCalc(x, ParRef.Lv.Sarc.SfPas,  step,options.NumSteps,scaling,params,'SfPas');
currentSarc.dLsPas = xStepCalc(x, ParRef.Lv.Sarc.dLsPas, step,options.NumSteps,scaling,params,'dLsPas');

Par.Lv.Sarc = currentSarc;
Par.Rv.Sarc = currentSarc;
Par.Sv.Sarc = currentSarc;
...
switch options.AutoScale
    ...
    otherwise
        Scale.TubeLArt = [Scale.f.TubeLArt,Scale.f.ValveLArt,Scale.f.TubeRArt,Scale.f.ValveRArt,Scale.Mean];
        Scale.TubeLArt = xStepCalc(Scale.TubeLArt(1),1,step,options.NumSteps);
        
        Scale.TubeRArt = [Scale.f.TubeRArt,Scale.f.ValveRArt,Scale.f.TubeLArt,Scale.f.ValveLArt,Scale.Mean];
        Scale.TubeRArt = xStepCalc(Scale.TubeRArt(1),1,step,options.NumSteps);
        
        Scale.ValveLArt = [Scale.f.ValveLArt,Scale.f.TubeLArt,Scale.f.ValveRArt,Scale.f.TubeRArt,Scale.Mean];
        Scale.ValveLArt = xStepCalc(Scale.ValveLArt(1),1,step,options.NumSteps);
        
        Scale.ValveRArt = [Scale.f.ValveRArt,Scale.f.TubeRArt,Scale.f.ValveLArt,Scale.f.TubeLArt,Scale.Mean];
        Scale.ValveRArt = xStepCalc(Scale.ValveRArt(1),1,step,options.NumSteps);
        
        Scale.ValveLAv = [Scale.f.ValveLAv,Scale.f.TubeRArt,Scale.Mean];
        Scale.ValveLAv = xStepCalc(Scale.ValveLAv(1),1,step,options.NumSteps);
        
        Scale.ValveRAv = [Scale.f.ValveRAv,Scale.f.TubeLArt,Scale.Mean];
        Scale.ValveRAv = xStepCalc(Scale.ValveRAv(1),1,step,options.NumSteps);
end
