function [y] = evalBiV(BiV,params,x,scaling,optIn)
% evalBiV   Evaluates CircAdapt for a given patient and set of parameters
%   evalBiV(BiV,params,x) evaluates patient "BiV" with a vector "x" of
%   values matching the structure of model parameters "params".
%     Run "paramsBiVRef()" to see a list of available parameters
%   
%   evalBiV(BiV,params,x,scaling) includes a vector "scaling" that is
%   multiplied by "x" before evaluation. This is useful when scaling
%   parameters for a solver or to control for sensitivity.
%   
%   evalBiV(BiV,params,x,scaling,optIn) includes a structure "optIn"
%   of options.
%     Run "evalBiV(BiV,params,x,scaling,{'avail'})" to see available
%     options.
%
%   Dependencies:
%     Need to put all CircAdapt files in ../worker# folder for each thread
...
if isempty(output) % Unique parameters
    ...
    if Par.Failure == 0
        ...
        [~] = CircSVarDot(0,transpose(Par.SVar),[]);
        
        CalP = 1;
        CalV = 1e6;
        
        switch options.UseDiastolePref
            case 'false'
                Pmax = max(CalP*Par.Lv.p); %pIn (Aortic entry vs LV)
            otherwise
                Pmax = min(CalP*Par.TubeLArt.p); % Aortic Diastole
        end
...
switch options.UseDiastolePref
    ...
    otherwise
        Perr = (Pmax-BiVstruct.refPd)/BiVstruct.refPd;
end
